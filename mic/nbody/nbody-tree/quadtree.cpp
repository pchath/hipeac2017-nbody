...
float QuadTree::computeNonEdgeForces(int point_index, float theta, float neg_f[])
{
	float sum_Q = 0;

	// Make sure that we spend no time on empty nodes or self-interactions
	if (cum_size == 0 || (is_leaf && size == 1 && index[0] == point_index)) return sum_Q;

	// Compute distance between point and center-of-mass
	float D = .0;
	int ind = point_index * QT_NO_DIMS;
	float local_buff[2] = { 0, 0 };
	for (int d = 0; d < QT_NO_DIMS; d++) local_buff[d] = data[ind + d];
	for (int d = 0; d < QT_NO_DIMS; d++) local_buff[d] -= center_of_mass[d];
	for (int d = 0; d < QT_NO_DIMS; d++) D += local_buff[d] * local_buff[d];
	...
	if (is_leaf || max(boundary.hh, boundary.hw) / sqrt(D) < theta) {
		...
		for (int d = 0; d < QT_NO_DIMS; d++) neg_f[d] += mult * local_buff[d];
	}
	else {

		// Recursively apply Barnes-Hut to children
		float nw_neg_f[2] = { 0, 0 }, ne_neg_f[2] = { 0, 0 }, sw_neg_f[2] = { 0, 0 }, se_neg_f[2] = { 0, 0 };
		sum_Q += northWest->computeNonEdgeForces(point_index, theta, nw_neg_f);
		sum_Q += northEast->computeNonEdgeForces(point_index, theta, ne_neg_f);
		sum_Q += southWest->computeNonEdgeForces(point_index, theta, sw_neg_f);
		sum_Q += southEast->computeNonEdgeForces(point_index, theta, se_neg_f);
		neg_f[0] = nw_neg_f[0] + ne_neg_f[0] + sw_neg_f[0] + se_neg_f[0];
		neg_f[1] = nw_neg_f[1] + ne_neg_f[1] + sw_neg_f[1] + se_neg_f[1];
	}
